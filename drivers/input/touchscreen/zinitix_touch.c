...
#include "zinitix_touch_zxt_firmware(ZI032210).h"
...
static DEVICE_ATTR(tsp_firm_version_panel, S_IRUGO | S_IWUSR | S_IWGRP, get_tsp_firm_version_panel, NULL);/*PART*/
...
#if	FIX_COOD_Y
	u16 fix_y[MAX_SUPPORTED_FINGER_NUM];
#endif
};
...
static bool ts_mini_init_touch(struct zinitix_touch_dev *touch_dev)
{
	...
	for(i=0;i<MAX_SUPPORTED_FINGER_NUM;i++)
        touch_dev->fix_y[i]= 10000;
#endif

	if (ts_write_reg(touch_dev->client, 0x003e,	1) != I2C_SUCCESS)
		goto fail_mini_init;
	if (ts_write_reg(touch_dev->client, 0x003f,	40) != I2C_SUCCESS)
		goto fail_mini_init;
	if (ts_write_reg(touch_dev->client, 0x00c6,	70) != I2C_SUCCESS)
		goto fail_mini_init;
	if (ts_write_reg(touch_dev->client, 0x00c7,	20) != I2C_SUCCESS)
		goto fail_mini_init;
	if (ts_write_reg(touch_dev->client, 0x00d4,	3000) != I2C_SUCCESS)
		goto fail_mini_init;
...
//				ABS_MT_WIDTH_MINOR, (u32)touch_dev->touch_info.coord[i].minor_width);
			report_angle = ((palm>1)?70:(touch_dev->touch_info.coord[i].angle - 90));
			input_report_abs(touch_dev->input_dev,
				ABS_MT_ANGLE, report_angle);
			zinitix_debug_msg("finger [%02d] angle = %03d, %03d\r\n", i, touch_dev->touch_info.coord[i].angle, report_angle);
...
static void get_chip_name(void *device_data)
{
	...
	snprintf(buff, sizeof(buff), "%s", info->cap_info.is_zmt200?"BT432":"BT532");
	set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
	info->cmd_state = 2;
	dev_info(&info->client->dev, "%s: %s(%d)\n", __func__,
			buff, strnlen(buff, sizeof(buff)));
...
static void run_reference_read(void *device_data)
{
	...
	for(i=0; i<info->cap_info.x_node_num; i++)
	{
		for(j=0; j<info->cap_info.y_node_num; j++)
		{
			...
			if(info->dnd_data[j+i*info->cap_info.y_node_num] < min)
			{
				if((i < info->cap_info.x_node_num-1))
					min = info->dnd_data[j+i*info->cap_info.y_node_num];
				else
			{
					if(j==3||j==14)
						min = info->dnd_data[j+i*info->cap_info.y_node_num];
			...
			if(info->dnd_data[j+i*info->cap_info.y_node_num] > max)
			{
				if((i < info->cap_info.x_node_num-1))
					max = info->dnd_data[j+i*info->cap_info.y_node_num];
				else
			{
					if(j==3||j==14)
						max = info->dnd_data[j+i*info->cap_info.y_node_num];
...
static int set_conifg_flip_cover(int enables)
{
    ...
    printk("[TSP] set_conifg_flip_cover : jig_attach_info = %d !!\n",jig_attach_info);

    if(jig_attach_info == 1)
    {
...
//		return 0;
//	}
	if (ts_write_reg(misc_touch_dev->client, ZINITIX_BTN_SENSTIVITY, 80) != I2C_SUCCESS) {
...
//		return 0;
//	}
	if (ts_write_reg(misc_touch_dev->client, ZINITIX_BTN_SENSTIVITY, 350) != I2C_SUCCESS) {
		printk(KERN_INFO "flip close btn sensitivity error..\n");
		enable_irq(misc_touch_dev->irq);
		misc_touch_dev->work_proceedure = TS_NO_WORK;		
		up(&misc_touch_dev->work_proceedure_lock);
